...
require 'connect.php';
try {
  $sql = 'SELECT lecturer.name as referee, students.name as student, company, position, job_location, lecturer_approved, id FROM reference, lecturer, students WHERE reference.stu_id = students.stu_id AND reference.lecturer_id = lecturer.lecturer_id';
  $q = $conn->query($sql);
